...
def run(conn, client, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Pipeline parameters
    EXCHANGES_LIST = params.get("exchanges_list", "Binance,Bybit,OKX")  # Comma-separated list
    SYMBOLS = params.get("symbols", ["BTC", "ETH", "SOL"])  # Trading coins
    INTERVALS = params.get("intervals", ["1h", "4h", "6h", "8h", "12h", "1d", "1w"])  # API intervals
    RANGES = params.get("ranges", ["0.5", "1", "2", "5"])  # Depth percentages
    ...
    logger.info(f"Starting Futures Aggregated Ask Bids History pipeline for exchanges: {EXCHANGES_LIST}")

    for symbol in SYMBOLS:
        for interval in INTERVALS:
            for range_percent in RANGES:
                try:
                    logger.info(f"Fetching futures aggregated ask bids history for {symbol} {interval} range={range_percent}")
                    ...
                    if data:
                        ...
                        result = repo.upsert_futures_aggregated_ask_bids_history_batch(
                            EXCHANGES_LIST, symbol, interval, range_percent, data
                        )
                        logger.info(
                            f"✅ futures_aggregated_ask_bids_history[{EXCHANGES_LIST}:{symbol}:{interval}:range={range_percent}]: "
                        ...
                        logger.info(
                            f"⚠️ futures_aggregated_ask_bids_history[{EXCHANGES_LIST}:{symbol}:{interval}:range={range_percent}]: No data (skipped)"
                        )
                    ...
                    logger.warning(
                        f"⚠️ futures_aggregated_ask_bids_history[{EXCHANGES_LIST}:{symbol}:{interval}:range={range_percent}]: Exception: {e} (skipped)"
                    )
                    summary["errors"] += 1
                    continue
