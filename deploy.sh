...
show_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  --all-profiles     Deploy all services (derivatives, exchange, spot, etf, trading)"
...
case "${1:-}" in
    --all-profiles)
        PROFILES="--profile derivatives --profile exchange --profile spot --profile etf --profile trading"
